...
// ── Shared type ───────────────────────────────────────────────────────────────
// Works for both DB Restaurant rows and Google Places results.
export interface ProfileTarget {
  id?:          string;       // DB restaurant UUID — present for verified DB cards only
  name:         string;
  city:         string;
  address?:     string | null;
  is_verified?: boolean;
...
export function RestaurantDetailModal({ restaurant, onClose }: Props) {
  ...
  // Reset toggles when a different restaurant opens
  useEffect(() => {
    setIsFav(false);
    setIsWish(false);
    setShowPrompt(false);
    if (!restaurant?.id) { setUserId(null); return; }
  ...
  const toggleFav = async () => {
    ...
    if (isFav) {
      await supabase.from("user_favorites").delete()
        .eq("user_id", userId).eq("restaurant_id", restaurant.id);
      setIsFav(false);
    } else {
  ...
  const toggleWish = async () => {
    if (!userId || !restaurant.id || wishLoading) return;
    setWishLoading(true);
    const supabase = createClient();
    if (isWish) {
  ...
  const canToggle = !!userId && !!restaurant.id;
  ...
  const modal = (
    <>
      ...
      {/* Panel — mobile drawer / desktop centered card */}
      <div
        className={[
      ...
      >
        ...
        <div style={{
          ...
          padding: "16px 20px", borderBottom: "1px solid rgb(var(--border))", flexShrink: 0,
        }}>
          <div style={{ flex: 1, minWidth: 0, paddingRight: "12px" }}>
            ...
            {/* ── Fav + Wishlist toggle buttons (only for DB restaurants) ── */}
            {restaurant.id && (
              <div style={{ display: "flex", gap: "8px", marginTop: "10px" }}>
                <button
                  onClick={toggleFav}
                  disabled={!canToggle || favLoading}
                  title={canToggle ? (isFav ? "Ukloni iz favorita" : "Dodaj u favorite") : "Prijavi se za favorite"}
                  style={{
                ...
                >
                  <Heart style={{ width: "12px", height: "12px", fill: isFav ? "#ef4444" : "transparent" }} />
                  {isFav ? "Favorit" : "Favoriti"}
                </button>

                <button
                  onClick={toggleWish}
                  disabled={!canToggle || wishLoading}
                  title={canToggle ? (isWish ? "Ukloni s wishiste" : "Dodaj na wishlstu") : "Prijavi se za wishlistu"}
                  style={{
                    display: "flex", alignItems: "center", gap: "5px",
                    padding: "5px 12px", borderRadius: "9999px", fontSize: "12px", fontWeight: 600,
                ...
                >
                  <Bookmark style={{ width: "12px", height: "12px", fill: isWish ? "#D35400" : "transparent" }} />
        ...
        <div style={{ flex: 1, overflowY: "auto", overscrollBehavior: "contain" }}>
          <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {/* ── Hero card ──────────────────────────────────────────────── */}
            <div style={{
              borderRadius: "16px", background: "rgb(var(--background))",
              border: "1px solid rgb(var(--border))", padding: "20px",
                  ...
                  <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                    {cleanTypes.map((t) => (
                      <span key={t} style={{
                        ...
                        background: "rgb(var(--border))", color: "rgb(var(--muted))", textTransform: "capitalize",
                      }}>{t}</span>
                    ))}
                  </div>
                )}
            ...
            {/* ── AI prompt concept ───────────────────────────────────────── */}
            <div style={{
              borderRadius: "16px", border: "1px solid rgba(139,92,246,0.25)",
              background: "rgba(139,92,246,0.05)", padding: "16px",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                <Sparkles style={{ width: "18px", height: "18px", color: "#8b5cf6", flexShrink: 0 }} />
                <p style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: "14px", color: "rgb(var(--foreground))", margin: 0 }}>
                  GENERIRAJ AI SLIKU
                </p>
              </div>
              {!showPrompt ? (
                <button
                  ...
                  style={{
                    ...
                    color: "#8b5cf6", fontWeight: 600, fontSize: "13px", cursor: "pointer",
                ...
                <div>
                  <p style={{ fontSize: "11px", color: "rgb(var(--muted))", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                    AI prompt spreman za kopiranje:
                  </p>
                  <div style={{
                    background: "rgb(var(--background))", border: "1px solid rgb(var(--border))",
                    borderRadius: "10px", padding: "12px", fontSize: "12px",
                    color: "rgb(var(--foreground))", lineHeight: 1.6, userSelect: "text",
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "10px" }}>
                    ...
                    <a
                      ...
                      style={{
                        ...
                        background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.3)",
                        color: "#8b5cf6", fontSize: "12px", fontWeight: 600, textDecoration: "none",
                        display: "flex", alignItems: "center", justifyContent: "center",
                      }}
                    >
                      Otvori Replicate →
                  ...
                  <p style={{ fontSize: "11px", color: "rgb(var(--muted))", margin: "8px 0 0", lineHeight: 1.5 }}>
                    Kopiraj prompt → idi na Replicate ili OpenAI DALL·E → zalijepi i generiraj.
                  </p>
                </div>
              )}
            </div>
            ...
            {/* ── Accommodation CTA ───────────────────────────────────────── */}
            <div style={{
              borderRadius: "16px", border: "1px solid rgba(211,84,0,0.25)",
              background: "rgba(211,84,0,0.06)", padding: "20px",
            }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "16px" }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: "16px", color: "rgb(var(--foreground))", margin: "0 0 4px" }}>
                    PLANIRAŠ POSJET?
                  </p>
...
function InfoRow({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      padding: "13px 16px", borderBottom: "1px solid rgb(var(--border))",
